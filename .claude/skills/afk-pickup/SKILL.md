...
`Closes #N: ...`. The repo squash-merges, so this title becomes the commit
subject on master, and release-please derives the version bump and the CHANGELOG
entry from it (PRD #498). The issue reference lives in the body's `## Closes`
section, which is what auto-closes the issue on merge; a `#N` in the title would
only pollute the changelog.
...
- `<type>` — inferred from the issue's nature: `feat` for a feature slice, `fix`
  ...
  team slice, use `feat`. Breaking change → append `!` (e.g. `feat(backend)!:`).
- `<scope>` — optional, lowercase, no spaces: the app or area most affected
  (`backend`, `frontend`, `smoker`, `device-service`, `agent`, `ci`, `infra`,
  `monorepo` when several).
