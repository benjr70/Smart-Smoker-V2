...
test_dev_serve() {
  ...
  # Test HTTP endpoint
  if curl -s -m 5 "http://smart-smoker-dev-cloud" &> /dev/null; then
  ...
  if curl -s -m 5 "http://smart-smoker-dev-cloud:3001" &> /dev/null; then
...
generate_summary() {
  ...
  print_info "  Dev HTTP:   http://smart-smoker-dev-cloud"
  print_info "  Dev WS:     http://smart-smoker-dev-cloud:3001"
