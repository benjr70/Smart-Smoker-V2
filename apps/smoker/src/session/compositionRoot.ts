...
/**
 * Loopback device-service origin for the temp feed. Used unless a per-stack URL
 * was baked into the bundle at build time (hermetic per-PR stacks publish
 * device-service on a remapped host port).
 */
const DEVICE_SERVICE_FALLBACK_URL = 'http://127.0.0.1:3003';
...
export function createSmokerSessionConfig(): SessionConfig {
  ...
  const config: SessionConfig = {
    ...
    deviceFeed: createDeviceFeedAdapter(resolveDeviceUrl(DEVICE_SERVICE_FALLBACK_URL)),
