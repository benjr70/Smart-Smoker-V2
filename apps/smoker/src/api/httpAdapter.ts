 ...
 * mutation, no per-call env read) and maps every failure to a typed
 * {@link ApiError} carrying status, path, method and the underlying cause. It
 * never resolves `undefined`: it returns `response.data` or throws.
 *
 * The smoker builds two of these — one per base URL (cloud API, device service)
 * — replacing the old per-call `axios.defaults.baseURL` mutation.
 */
...
export const createHttpTransport = (baseURL: string | undefined): TransportPort => {
