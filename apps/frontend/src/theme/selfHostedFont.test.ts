/**
 * The typeface has to come out of the app's own bundle.
 *
 * The smoker is reachable over a tailnet and may have no route to the public
 * internet, so a stylesheet fetched from a font CDN would simply never arrive
 * and the interface would render in a fallback face. These tests fail if the
 * webfont stops being a bundled dependency, if the bundler stops emitting its
 * files, or if a remote font host creeps back into the app's pages.
 */
import fs from 'fs';
...
describe('the design typeface is served by the application itself', () => {
