...
/** A cook event as JSON carries it: the moment is a string, since JSON has no date. */
export type WireCookEvent = Omit<CookEvent, 'at' | 'tone'> & {
  at: string | Date;
  tone: string;
 ...
 * a sequence — markers along a chart, a story under it — and no screen should
 * have to know which order the collection was asked for. A row carrying no
 * readable moment is dropped rather than plotted at the epoch: an event nobody
 * can place in the cook is worse than one that is missing.
 */
