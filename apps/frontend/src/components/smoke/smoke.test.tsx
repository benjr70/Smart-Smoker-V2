...
jest.mock('@mui/material', () => ({
  Stepper: ({ children, ...props }: any) => (
    <div data-testid="stepper" data-active-step={props.activeStep} {...props}>{children}</div>
  ),
  Step: ({ children, ...props }: any) => (
    <div data-testid="step" {...props}>{children}</div>
  ),
  StepButton: ({ children, onClick, ...props }: any) => (
    <button data-testid="step-button" onClick={onClick} {...props}>{children}</button>
  ),
  Button: ({ children, onClick, ...props }: any) => (
    <button data-testid="button" onClick={onClick} {...props}>{children}</button>
  ),
  Grid: ({ children, className, ...props }: any) => (
    <div data-testid="grid" data-classname={className} {...props}>{children}</div>
  )
}));

// Mock step components
jest.mock('./preSmokeStep/preSmokeStep', () => ({
  PreSmokeStep: ({ nextButton }: any) => (
    <div data-testid="pre-smoke-step">{nextButton}</div>
  )
}));

jest.mock('./smokeStep/smokeStep', () => ({
  SmokeStep: ({ nextButton }: any) => (
    <div data-testid="smoke-step">{nextButton}</div>
  )
}));

jest.mock('./postSmokeStep/PostSmokeStep', () => ({
  PostSmokeStep: ({ nextButton }: any) => (
    <div data-testid="post-smoke-step">{nextButton}</div>
  )
}));
