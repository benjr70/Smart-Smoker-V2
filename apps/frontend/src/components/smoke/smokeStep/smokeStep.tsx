...
export function SmokeStepView(props: SmokeStepProps): JSX.Element {
  ...
  // The cook that is set up right now, in one read: when it started, as the
  // backend stamped it, when it will be done, as the backend projects it, and
  // the probe it is being taken to. Re-read whenever smoking is switched —
  // that is the moment the stamp is written, and one of the two things this
  ...
  return (
    ...
    <Grid item xs={12} sx={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      <SmokeStatusBar smoking={session.smoking} startedAt={cook.startedAt} />
